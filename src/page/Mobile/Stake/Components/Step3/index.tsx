...
const Index = () => {
  ...
  return <>
    {
      account && stakeList && stakeList.length > 0 && <div className='m-stake-step1 m-stake-step3'>
        <h1 className='m-stake-step1-title'>{ts.step}<span>3</span>{ts.title}</h1>
       {
         stakeList.map((item: any, i: number) => {
          return <ContainerBg className='m-stake-step3-cont' key={i}>
            <header>
            ...
            <Button loading={loading && index === i} size='medium' onClick={() => handleWithdraw(item, i)}>{ts.withdraw}</Button>
        </ContainerBg>
        })
       }
      </div>
